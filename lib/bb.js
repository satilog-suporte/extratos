...
//
// Reutiliza o mesmo certificado da Matriz Bradesco — mesmas variáveis:
//   CERTIFICADO_PFX_BASE64_MATRIZ  → certificado A1 .pfx em base64
//   SENHA_CERTIFICADO_MATRIZ       → senha do .pfx
// ─────────────────────────────────────────────
...
function loadHttpsAgent() {
  ...
  const pfxBase64  = process.env.CERTIFICADO_PFX_BASE64_MATRIZ;
  const passphrase = process.env.SENHA_CERTIFICADO_MATRIZ;

  if (!pfxBase64) {
    throw new Error(
      'Certificado mTLS ausente. Defina CERTIFICADO_PFX_BASE64_MATRIZ com o certificado A1 ' +
      'em base64. A API de Extratos do BB exige mTLS em toda chamada.'
    );
  }
